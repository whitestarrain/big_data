...
- api:
  - `sklearn.model_selection.cross_val_score(estimator,data,target,cv,[scoring])`:获取 cv 折交叉验证的分数
...
- 随机森林创建过程：
  ...
  1. 单棵树创建过程
  1. 随机在 M 个特征中选出 m 个特征,m 应远小于 M
  1. 随机从 N 个样本中选择一个样本，重复 N 次。随机放回抽样(bootstrap 抽样),样本有可能重复
  1. 使用抽取的样本，训练模型,并用未抽到的样本作预测，评估其误差。
  1. 重复操作，建立指定数量的决策树
